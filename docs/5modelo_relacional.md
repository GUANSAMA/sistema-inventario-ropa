...
El modelo está implementado inicialmente en `database/02_crear_tablas.sql`, `03_insertar_datos_prueba.sql`, `04_crear_procedimientos.sql` y `05_crear_triggers_auditoria.sql`. El orden de ejecución es `01_crear_base_datos.sql`, `02_crear_tablas.sql`, `03_insertar_datos_prueba.sql`, `04_crear_procedimientos.sql` y `05_crear_triggers_auditoria.sql`. Los datos de prueba se insertan antes de crear los triggers y no se consideran operaciones realizadas por usuarios; por eso no aparecen en `AuditoriaInventario`. Las altas, modificaciones y desactivaciones hechas desde la aplicación sí pasan por los procedimientos y triggers. Antes de considerarlo validado, se deben ejecutar los scripts y comprobar las restricciones y operaciones en SQL Server.
